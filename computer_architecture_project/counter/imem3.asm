# ==============================================================================
# Project: Multi-Core Cache Simulator (MIPS-like)
# File:    new_counter/imem3.asm
# Author:
# ID:
# Date:    11/11/2024
#
# Description:
# Core 3 implementation of a shared counter update.
# Uses modulo-based turn checking (Counter % 4 == 3).
# ==============================================================================

# ------------------------------------------------------------------------------
# REGISTER MAP
# ------------------------------------------------------------------------------
# $r2: Address of Shared Counter (0x0)
# $r3: My Core ID (3)
# $r4: Max Count (512)
# $r5: Mask for Modulo 4 (3)
# $r6: Current Counter Value
# $r7: Sync Address (512)
# $r8: Temporary result
# ------------------------------------------------------------------------------

# ------------------------------------------------------------------------------
# INITIALIZATION
# ------------------------------------------------------------------------------
add, $r2, $zero, $zero, 0      # R2 = 0
add, $r3, $zero, $imm, 3       # R3 = 3 (My Core ID)
add, $r4, $zero, $imm, 512     # R4 = 512
add, $r5, $zero, $imm, 3       # R5 = 3
add, $r7, $zero, $imm, 512     # R7 = 512

# ------------------------------------------------------------------------------
# MAIN LOOP
# ------------------------------------------------------------------------------
# PC=5: Load current counter
lw, $r6, $r2, $zero, 0         

# PC=6: Check termination
beq, $imm, $r6, $r4, 12

# PC=7: Check Turn (Counter % 4 == 3)
and, $r8, $r6, $r5, 0
bne, $imm, $r8, $r3, 5

# ------------------------------------------------------------------------------
# CRITICAL SECTION
# ------------------------------------------------------------------------------
# PC=9: Increment
add, $r6, $r6, $imm, 1

# PC=10: Store
sw, $r6, $r2, $zero, 0

# PC=11: Loop
beq, $imm, $zero, $zero, 5

# ------------------------------------------------------------------------------
# TERMINATION
# ------------------------------------------------------------------------------
# PC=12: Final Sync
lw, $r8, $r7, $zero, 0
halt, $zero, $zero, $zero, 0
